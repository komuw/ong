...
func TestCsrf(t *testing.T) {
	...
	t.Run("middleware succeds", func(t *testing.T) {
		...
		msg := "hello"
		domain := "example.com"
		wrappedHandler := csrf(someCsrfHandler(msg), tst.SecretKey(), domain, DefaultCsrfCookieMaxDuration)
	...
	t.Run("POST requests with valid token one tab", func(t *testing.T) {
		...
		key := tst.SecretKey()
		enc2 := cry.New(key)
		expires := strconv.FormatInt(time.Now().UTC().Add(DefaultCsrfCookieMaxDuration).Unix(), 10)
