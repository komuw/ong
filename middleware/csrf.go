...
func Csrf(wrappedHandler http.HandlerFunc, secretKey []byte, domain string) http.HandlerFunc {
	enc, err := NewEnc(secretKey)
	if err != nil {
		panic(err)
	}
	secretKey = nil // destroy the key.
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		default:
			...
			errN := validateToken(enc, actualToken)
			if errN != nil {
...
func validateToken(enc *enc, actualToken string) error {
	encryptedMsg, err := decode(actualToken)
	if err != nil {
		return err
	}

	_, err = enc.Decrypt(encryptedMsg)
	return err
}
