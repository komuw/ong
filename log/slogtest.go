...
//
// This func is copied from https://github.com/golang/go/blob/go1.21.0/src/testing/slogtest/slogtest.go#L51
...
func testHandler(h slog.Handler, results func() []map[string]any) error {
	cases := []testCase{
		...
		},
		{
			...
			f: func(l *slog.Logger) {
				l.Error("msg", "a", "b", slog.Group("G", slog.String("c", "d")), "e", "f")
			},
			checks: []check{
				hasAttr("a", "b"),
		...
		},
		{
			...
			f: func(l *slog.Logger) {
				l.Error("msg", "a", "b", slog.Group("G"), "e", "f")
			},
			checks: []check{
				hasAttr("a", "b"),
				missingKey("G"),
		...
		},
		{
			...
			f: func(l *slog.Logger) {
				l.Error("msg", "a", "b", slog.Group("", slog.String("c", "d")), "e", "f")
			},
			checks: []check{
				hasAttr("a", "b"),
				hasAttr("c", "d"),
		...
		},
		{
			...
			f: func(l *slog.Logger) {
				l.With("a", "b").WithGroup("G").With("c", "d").WithGroup("H").Error("msg", "e", "f")
			},
			checks: []check{
				hasKey(slog.TimeKey),
				hasKey(slog.LevelKey),
		...
		},
		{
			...
			f: func(l *slog.Logger) {
				l.With("a", "b").WithGroup("G").With("c", "d").WithGroup("H").Error("msg")
			},
			checks: []check{
				hasKey(slog.TimeKey),
				hasKey(slog.LevelKey),
				...
				hasAttr("a", "b"),
				inGroup("G", hasAttr("c", "d")),
				// Has to disable this check since our logger includes logID for all logs
		...
		},
		{
			...
			f: func(l *slog.Logger) {
				l.Error("msg", "k", &replace{"replaced"})
			},
